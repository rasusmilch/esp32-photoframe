...
Fixtures additionally cover truncated active/stopped/fenced epochs, missing/duplicate/premature completion, evidence-loss faults, exact fence-context mismatches, symbolic-event mismatches, repeated stop, rapid-replacement errors, explicit APSTA configuration, and records after completion. Passing fixtures now cover scenarios A–G and enforce scenario identity/phases, required outcomes, per-epoch AP/STA configuration, and the post-fence quarantine. Negative fixtures cover premature success, late/wrong-owner replacement, missing APSTA milestones, wrong AP restart mode, missing timeout, driver events during quarantine, stale queued starts after faults, and release/start/completion before quiet observation. Fixtures now verify mode-specific stop masks, APSTA two-event completion, duplicate/inapplicable stop rejection, active transition AP-stop behavior, exactly one fence, absolute deadline consistency, 1,999 ms rejection for a 2,000 ms quarantine, ordered epoch-qualified scenarios, phase/action consistency, and truncation between every stop/fence/quarantine/release boundary. Every pass fixture terminates with `run_complete` and is tested after multiple deliberate truncations. Its E1002/E1004 repeated physical matrix remains pending; checker success does not validate ESP-IDF ordering or the proposed fence.
